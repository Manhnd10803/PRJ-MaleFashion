<div class="header__top">
    <div class="container">
        <div class="row">
            <div class="col-lg-6 col-md-7">
                <div class="header__top__left">
                    <p>Free shipping, 30-day return or refund guarantee.</p>
                </div>
            </div>
            <div class="col-lg-6 col-md-5">
                <div class="header__top__right">
                    <div class="header__top__hover">
                        <span>Usd <i class="arrow_carrot-down"></i></span>
                        <ul>
                            <li>USD</li>
                            <li>EUR</li>
                            <li>USD</li>
                        </ul>
                    </div>
                    <div class="header__top__links">
                        <a href="#">FAQs</a>
                        @if (Auth::check())
                        <div class="header__top__hover">
                            <span>Hi, {{ Auth::user()->username }} <i class="arrow_carrot-down"></i></span>
                            <ul>
                                <a href="{{ route('logout') }}"><li>Logout</li></a>
                                @if (Auth::user()->role == 1)
                                    <a href="{{ route('admin.dashboard') }}"><li>Admin</li></a>
                                @endif
                            </ul>
                        </div>
                        @else
                            <a href="{{ route('login') }}">Sign in</a>
                        @endif
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
<div class="container">
    <div class="row">
        <div class="col-lg-3 col-md-3">
            <div class="header__logo">
                <a href="/"><img src="{{ asset('storage/img/logo.png') }}" alt=""></a>
            </div>
        </div>
        <div class="col-lg-6 col-md-6">
            <nav class="header__menu mobile-menu">
                <ul>
                    <li><a href="/">Home</a></li>
                    <li><a href="{{ route('listProduct') }}">Shop</a></li>
                    <li><a href="{{ route('listOrder') }}">Order</a>
                        {{-- <ul class="dropdown">
                            <li><a href="./about.html">About Us</a></li>
                            <li><a href="./shop-details.html">Shop Details</a></li>
                            <li><a href="./shopping-cart.html">Shopping Cart</a></li>
                            <li><a href="./checkout.html">Check Out</a></li>
                            <li><a href="./blog-details.html">Blog Details</a></li>
                        </ul> --}}
                    </li>
                    <li><a href="./blog.html">Blog</a></li>
                    <li><a href="./contact.html">Contacts</a></li>
                </ul>
            </nav>
        </div>
        <div class="col-lg-3 col-md-3">
            <div class="header__nav__option">
                <a href="#" class="search-switch"><img src="{{ asset('storage/img/icon/search.png') }}" alt=""></a>
                <a href="#"><img src="{{ asset('storage/img/icon/heart.png') }}" alt=""></a>
                <a href="{{ route('viewCart') }}"><img src="{{ asset('storage/img/icon/cart.png') }}" alt=""> <span></span></a>
                <div class="price"></div>
            </div>
        </div>
    </div>
    <div class="canvas__open"><i class="fa fa-bars"></i></div>
</div>